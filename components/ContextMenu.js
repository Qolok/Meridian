import { assignTab, unassignTab, createWorkspace } from '../utils/workspaceManager.js';
...
function item(label, onClick) {
  ...
  btn.addEventListener('click', (e) => { e.stopPropagation(); hide(); onClick(); });
  return btn;
}
...
export async function show(tab, x, y) {
  ...
  // Move to new group
  menu.appendChild(item('Move to new group', async () => {
  ...
  if (inChromeGroup || wsId) {
    menu.appendChild(item('Remove from group', async () => {
      if (hasNativeGroups && inChromeGroup) await chrome.tabs.ungroup([tab.id]).catch(() => {});
      await unassignTab(tab.id);
    }));
  }
  ...
  if (otherChromeGroups.length > 0) {
    ...
    for (const g of otherChromeGroups) {
      ...
      menu.appendChild(item(`Move to "${name}"`, async () => {
  ...
  if (otherWorkspaces.length > 0) {
    ...
    for (const ws of otherWorkspaces) {
      menu.appendChild(item(`Move to "${ws.name}"`, async () => {
        if (hasNativeGroups && inChromeGroup) await chrome.tabs.ungroup([tab.id]).catch(() => {});
        await assignTab(tab.id, ws.id);
      }));
    }
