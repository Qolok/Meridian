...
async function reorderInChromeGroup(draggedTabId, refTabId, groupId) {
  ...
  const insertIdx = refTabId != null ? newOrder.findIndex(t => t.id === refTabId) : newOrder.length;
  newOrder.splice(insertIdx === -1 ? newOrder.length : insertIdx, 0, groupTabs.find(t => t.id === draggedTabId));
  const newRelIdx = newOrder.findIndex(t => t.id === draggedTabId);
  await chrome.tabs.move(draggedTabId, { index: groupTabs[0].index + newRelIdx });
}

async function reorderInStorage(draggedTabId, refTabId, workspaceId, currentTabIds) {
  const newOrder = currentTabIds.filter(id => id !== draggedTabId);
  const insertIdx = refTabId != null ? newOrder.indexOf(refTabId) : newOrder.length;
  newOrder.splice(insertIdx === -1 ? newOrder.length : insertIdx, 0, draggedTabId);
...
export function createWorkspaceLane(workspace, tabs, thumbnails, onTabClosed, { chromeGroup, meridianWorkspace, collapsed: initialCollapsed = false } = {}) {
  ...
  function liveItems() {
    return [...grid.children].filter(c => c !== dropPlaceholder && !c.classList.contains('dragging'));
  }

  function applyFlip(items, first) {
    const last = new Map(items.map(c => [c, c.getBoundingClientRect()]));
    for (const item of items) {
      const f = first.get(item), l = last.get(item);
      if (!f || !l) continue;
  ...
  function commitReorder(draggedTabId) {
    ...
    if (chromeGroup) return reorderInChromeGroup(draggedTabId, refTabId, chromeGroup.id);
  ...
  function movePlaceholder(insertRef) {
    ...
    if (draggedEl) {
      ...
      if (naturalNext === dropPlaceholder) naturalNext = naturalNext.nextElementSibling;
    ...
    if (dropPlaceholder.parentNode === grid && dropPlaceholder.nextElementSibling === insertRef) return;
    if (insertRef === lastInsertRef && dropPlaceholder.parentNode === grid) return;
  ...
  for (const tab of tabs) {
    ...
    if (isPlaceholder) {
      ...
      card.addEventListener('dragover', (e) => {
        if (dragState.laneId !== workspace.id || dragState.tabId === tab.id) return;
        e.preventDefault();
        e.stopPropagation();
        const rect = card.getBoundingClientRect();
        let ref = e.clientX >= rect.left + rect.width / 2
  ...
  lane.addEventListener('drop', async (e) => {
    ...
    if (dragState.laneId === workspace.id && dropPlaceholder?.parentNode === grid) {
