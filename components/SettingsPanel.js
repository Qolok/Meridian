...
const GRADIENT_PRESETS = [
  { id: 'g1', value: 'linear-gradient(135deg,#1a1a2e,#16213e,#0f3460)', label: 'Midnight' },
  { id: 'g2', value: 'linear-gradient(135deg,#0f2027,#203a43,#2c5364)', label: 'Ocean' },
  { id: 'g3', value: 'linear-gradient(135deg,#667eea,#764ba2)',          label: 'Dusk' },
  { id: 'g4', value: 'linear-gradient(135deg,#11998e,#38ef7d)',          label: 'Emerald' },
  { id: 'g5', value: 'linear-gradient(135deg,#f7971e,#ffd200)',          label: 'Amber' },
  { id: 'g6', value: 'linear-gradient(135deg,#f093fb,#f5576c)',          label: 'Bloom' },
];
...
export function createSettingsPanel(container, onClose) {
  ...
  function renderThemeButtons() {
    themeRow.querySelectorAll('.settings-theme-btn').forEach(el => el.remove());
    for (const t of THEMES) {
      const btn = document.createElement('button');
      btn.className = 'settings-theme-btn' + (t.id === currentTheme ? ' selected' : '');
  ...
  function makeSwatch(opts) {
    const btn = document.createElement('button');
    btn.className = 'settings-bg-swatch' +
    ...
    if (opts.imgSrc) {
      ...
      img.alt = '';
      img.loading = 'lazy';
      img.onerror = () => { btn.style.background = 'var(--surface-hover)'; };
      btn.appendChild(img);
    }
  ...
  function renderBgSection() {
    ...
    // None
    combinedGrid.appendChild(makeSwatch({
      isNone:   true,
    ...
    for (const seed of photoSeeds) {
      ...
      photoGrid.appendChild(makeSwatch({
    ...
    uploadBtn.className = 'settings-bg-upload-btn' +
      (currentBg.type === 'custom' ? ' selected' : '');
    uploadBtn.textContent = currentBg.type === 'custom'
  ...
  chrome.storage.sync.get(['newTabBehavior', 'groupByDomain', 'homepageUrl', 'theme', 'background']).then((saved) => {
